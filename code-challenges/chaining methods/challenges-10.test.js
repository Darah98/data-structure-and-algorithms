...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map((bulkArr) => {
    return bulkArr
      .filter((numVal) => {
        if (typeof numVal === 'number' && numVal % 5 === 0) {
          return numVal;
        }
      })
      .map((power) => {
...
let findMaleAndFemale = (data) => {
  let newArr = data
    .filter((obj) => {
      if (obj.gender === 'female' || obj.gender === 'male') return obj;
    })
    .map((gen) => {
      return gen.name;
    })
...
let findShortest = (data) => {
  return data.reduce((shortest, obj) => {
